...
const CSS = `
  ...
  .topic-row:hover{background:rgba(240,237,232,0.04)!important;border-left-color:#BE473D!important;}
    ...
    <PageLayout title="FINANCE GUIDE" subtitle="// MONEY SKILLS FOR FRESH GRADS">
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'4px 12px 4px 8px',border:'1px solid rgba(240,237,232,0.1)',marginBottom:14}}>
          ...
          <span style={{fontSize:9,letterSpacing:2,color:'rgba(240,237,232,0.38)'}}>SALARY CALCULATOR</span>
        </div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input type="number" value={salary} onChange={function(e){setSalary(e.target.value)}} placeholder="e.g. 30000"
            style={{width:'100%',padding:'13px 14px',background:'rgba(240,237,232,0.05)',border:'1px solid rgba(240,237,232,0.1)',color:'#F0EDE8',fontFamily:MONO,fontSize:14,letterSpacing:.5,transition:'border-color .18s'}}
          ...
          <div>
            ...
            <div style={{border:'1px solid rgba(240,237,232,0.07)',borderBottom:'none',marginBottom:1}}>
              {[
                ...
                return(
                  <div key={d.l} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'11px 14px',borderBottom:'1px solid rgba(240,237,232,0.05)'}}>
                    <span style={{fontFamily:'monospace',fontSize:10,color:'rgba(240,237,232,0.38)'}}>{d.l}</span>
                    <span style={{fontFamily:MONO,fontSize:11,color:d.color,letterSpacing:.5}}>- ₱{d.v.toLocaleString()}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',background:'rgba(190,71,61,0.07)',border:'1px solid rgba(190,71,61,0.2)',marginBottom:1}}>
              <span style={{fontFamily:MONO,fontSize:10,color:'rgba(240,237,232,0.5)',letterSpacing:1}}>TOTAL DEDUCTIONS</span>
              <span style={{fontFamily:MONO,fontSize:13,color:'#BE473D',letterSpacing:.5}}>- ₱{deductions.total.toLocaleString()}</span>
            </div>

            {/* Take-home */}
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px',background:'rgba(240,237,232,0.04)',border:'1px solid rgba(240,237,232,0.1)',marginBottom:20}}>
              <span style={{fontFamily:MONO,fontSize:10,color:'rgba(240,237,232,0.6)',letterSpacing:1}}>MONTHLY TAKE-HOME</span>
              <span style={{fontFamily:MONO,fontSize:18,color:'#F0EDE8',letterSpacing:'-0.5px'}}>₱{takehome.toLocaleString()}</span>
            </div>
            ...
            <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'4px 12px 4px 8px',border:'1px solid rgba(240,237,232,0.1)',marginBottom:12}}>
              <span style={{fontSize:9,color:'#BE473D'}}>50/30/20</span>
              <span style={{fontSize:9,letterSpacing:2,color:'rgba(240,237,232,0.38)'}}>BUDGET BREAKDOWN</span>
            </div>
              ...
              {[
                ...
                return(
                  <div key={b.l} style={{padding:'14px 12px',background:'#3C091E',position:'relative',overflow:'hidden'}}>
                    ...
                    <div style={{fontFamily:MONO,fontSize:7.5,color:'rgba(240,237,232,0.25)',letterSpacing:1.5,marginBottom:6,lineHeight:1.3}}>{b.l}</div>
                    <div style={{fontFamily:MONO,fontSize:16,color:'#F0EDE8',letterSpacing:'-0.5px',marginBottom:4}}>₱{b.v.toLocaleString()}</div>
                    <div style={{fontFamily:'monospace',fontSize:8,color:'rgba(240,237,232,0.2)',lineHeight:1.5}}>{b.sub}</div>
                  </div>
                )
              })}
            </div>
      ...
      <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'4px 12px 4px 8px',border:'1px solid rgba(240,237,232,0.1)',marginBottom:14}}>
        ...
        <span style={{fontSize:9,letterSpacing:2,color:'rgba(240,237,232,0.38)'}}>MONEY TOPICS</span>
      </div>
        ...
        {TOPICS.map(function(t){
          return(
            <button key={t.n} className="topic-row" onClick={function(){navigate('/dashboard/chat',{state:{initialMessage:t.prompt}})}}
              style={{width:'100%',display:'flex',alignItems:'center',gap:14,padding:'14px 12px 14px 14px',borderBottom:'1px solid rgba(240,237,232,0.06)',background:'transparent',border:'none',borderBottom:'1px solid rgba(240,237,232,0.06)',cursor:'pointer',textAlign:'left'}}>
              <span style={{fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.18)',letterSpacing:1,width:22,flexShrink:0}}>{t.n}</span>
              <div style={{flex:1,minWidth:0}}>
                <div className="topic-title" style={{fontFamily:MONO,fontSize:11,color:'rgba(240,237,232,0.65)',letterSpacing:.5,marginBottom:3,transition:'color .18s'}}>{t.title}</div>
                <div style={{fontFamily:'monospace',fontSize:9.5,color:'rgba(240,237,232,0.22)',lineHeight:1.5}}>{t.sub}</div>
              </div>
              <span style={{fontFamily:MONO,fontSize:14,color:'rgba(240,237,232,0.15)',flexShrink:0}}>›</span>
            </button>
          )
        })}
      </div>
