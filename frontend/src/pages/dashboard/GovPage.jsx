...
const CSS = `
  ...
  .gov-row:hover{background:rgba(250,247,242,0.06)!important;border-left-color:#C8A84B!important;}
  .gov-row:hover .gov-name{color:#FAF7F2!important;}
...
var SC={done:{label:'DONE',color:'#34D399',border:'rgba(52,211,153,0.3)'},in_progress:{label:'IN PROGRESS',color:'#FBBF24',border:'rgba(251,191,36,0.3)'},not_started:{label:'NOT STARTED',color:'rgba(250,247,242,0.25)',border:'rgba(250,247,242,0.1)'}}
...
function GovRow({item,status,onCycle,navigate}){
  ...
  var cfg=SC[status]
  return(
      <div style={{borderBottom:'1px solid rgba(250,247,242,0.05)'}}>
        <div className="gov-row" onClick={function(){setOpen(!open)}} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 12px 14px 14px',cursor:'pointer',background:'transparent'}}>
          <div style={{width:36,height:36,background:'rgba(200,168,75,0.1)',border:'1px solid rgba(200,168,75,0.18)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,flexShrink:0}}>{item.icon}</div>
          <div style={{flex:1,minWidth:0}}>
            <div className="gov-name" style={{fontFamily:MONO,fontSize:14,color:'rgba(250,247,242,0.85)',letterSpacing:.5,marginBottom:3,transition:'color .18s'}}>{item.name}</div>
            <div style={{fontFamily:'monospace',fontSize:11,color:'rgba(250,247,242,0.42)'}}>{item.desc} · {item.time} · {item.fee}</div>
          </div>
          ...
          <span style={{fontFamily:MONO,fontSize:16,color:'rgba(250,247,242,0.28)',flexShrink:0}}>{open?'∨':'›'}</span>
        </div>
        {open&&(
            <div style={{padding:'0 12px 16px 62px'}}>
              <div style={{marginBottom:10}}>
                <div style={{fontFamily:MONO,fontSize:9,color:'rgba(250,247,242,0.35)',letterSpacing:2,marginBottom:6}}>// REQUIREMENTS</div>
                {item.requirements.map(function(r){return<div key={r} style={{fontFamily:'monospace',fontSize:12,color:'rgba(250,247,242,0.55)',lineHeight:1.7}}>› {r}</div>})}
              </div>
              <button onClick={function(){setSteps(!steps)}} style={{background:'transparent',border:'none',cursor:'pointer',fontFamily:MONO,fontSize:10,color:'#C8A84B',letterSpacing:1,padding:0,marginBottom:steps?10:0}} onMouseEnter={function(e){e.currentTarget.style.opacity='.7'}} onMouseLeave={function(e){e.currentTarget.style.opacity='1'}}>{steps?'─ HIDE GUIDE':'+ SHOW STEP-BY-STEP GUIDE'}</button>
              {steps&&(
                  <div style={{marginBottom:12}}>
                    {item.steps.map(function(step,i){return(
                        <div key={i} style={{display:'flex',gap:10,alignItems:'flex-start',marginBottom:6}}>
                          <div style={{width:20,height:20,background:'#C8A84B',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:MONO,fontSize:9,color:'#FAF7F2',flexShrink:0}}>{i+1}</div>
                          <div style={{fontFamily:'monospace',fontSize:12,color:'rgba(250,247,242,0.55)',lineHeight:1.65}}>{step}</div>
                        </div>
                    )})}
                  </div>
              )}
              <div style={{display:'flex',gap:8}}>
                <button onClick={function(){navigate('/dashboard/chat',{state:{initialMessage:item.kuyaPrompt}})}} style={{flex:1,padding:'11px',background:'#C8A84B',border:'none',cursor:'pointer',fontFamily:MONO,fontSize:10,color:'#FAF7F2',letterSpacing:1,transition:'opacity .18s'}} onMouseEnter={function(e){e.currentTarget.style.opacity='.85'}} onMouseLeave={function(e){e.currentTarget.style.opacity='1'}}>ASK KUYA AI</button>
                <button onClick={function(){window.open(item.link,'_blank','noopener,noreferrer')}} style={{flex:1,padding:'11px',background:'transparent',border:'1px solid rgba(250,247,242,0.07)',cursor:'pointer',fontFamily:MONO,fontSize:10,color:'rgba(250,247,242,0.5)',letterSpacing:1,transition:'all .18s'}} onMouseEnter={function(e){e.currentTarget.style.color='#FAF7F2';e.currentTarget.style.borderColor='rgba(250,247,242,0.3)'}} onMouseLeave={function(e){e.currentTarget.style.color='rgba(250,247,242,0.5)';e.currentTarget.style.borderColor='rgba(250,247,242,0.1)'}}>OFFICIAL SITE</button>
              </div>
            </div>
        )}
      </div>
  ...
  return(
      <PageLayout title="GOV REGISTRATIONS" subtitle="// YOUR POST-GRAD CHECKLIST">
        ...
        <div className="gov-bg" aria-hidden="true">
          ...
          <div style={{position:'absolute',top:'18%',left:'50%',transform:'translateX(-50%)',width:'min(90vw,600px)',height:'min(90vw,600px)',borderRadius:'50%',background:'radial-gradient(circle,rgba(200,168,75,0.14) 0%,rgba(200,168,75,0.04) 40%,transparent 70%)',animation:'gov-pulse 6s ease-in-out infinite'}}/>
          ...
          <div style={{position:'absolute',bottom:'10%',left:'-8%',width:200,height:200,borderRadius:'50%',background:'radial-gradient(circle,rgba(200,168,75,0.07) 0%,transparent 65%)'}}/>
          {/* Floating diamond — top right */}
          <div style={{position:'absolute',top:'8%',right:'6%',width:44,height:44,border:'1px solid rgba(200,168,75,0.18)',transform:'rotate(45deg)',animation:'gov-drift-a 11s ease-in-out infinite'}}/>
          {/* Floating diamond — bottom left */}
          <div style={{position:'absolute',bottom:'22%',left:'4%',width:28,height:28,border:'1px solid rgba(250,247,242,0.07)',transform:'rotate(45deg)',animation:'gov-drift-b 8s ease-in-out infinite 1s'}}/>
          {/* Floating ring */}
          <div style={{position:'absolute',top:'38%',right:'3%',width:56,height:56,borderRadius:'50%',border:'1px solid rgba(200,168,75,0.1)',animation:'gov-drift-b 13s ease-in-out infinite 2s'}}/>
          {/* Corner bracket — top left */}
          <div style={{position:'absolute',top:16,left:16,width:32,height:32,borderTop:'1px solid rgba(200,168,75,0.22)',borderLeft:'1px solid rgba(200,168,75,0.22)'}}/>
          {/* Corner bracket — bottom right */}
          <div style={{position:'absolute',bottom:16,right:16,width:32,height:32,borderBottom:'1px solid rgba(200,168,75,0.22)',borderRight:'1px solid rgba(200,168,75,0.22)'}}/>
          {/* Diagonal rule lines */}
          <div style={{position:'absolute',top:'30%',left:0,width:'35%',height:1,background:'linear-gradient(90deg,transparent,rgba(200,168,75,0.08),transparent)',transform:'rotate(-8deg)',transformOrigin:'left'}}/>
          <div style={{position:'absolute',bottom:'30%',right:0,width:'28%',height:1,background:'linear-gradient(90deg,transparent,rgba(250,247,242,0.05),transparent)',transform:'rotate(6deg)',transformOrigin:'right'}}/>
          {/* Binary ambient text */}
          <div style={{position:'absolute',top:'12%',left:'2%',fontFamily:'monospace',fontSize:8,color:'rgba(250,247,242,0.055)',letterSpacing:2,lineHeight:2,userSelect:'none'}}>{'01001010\n10110100\n00101101'}</div>
          <div style={{position:'absolute',bottom:'14%',right:'2%',fontFamily:'monospace',fontSize:8,color:'rgba(250,247,242,0.045)',letterSpacing:2,lineHeight:2,userSelect:'none',textAlign:'right'}}>{'10011010\n01100101'}</div>
          {/* Scanline */}
          <div style={{position:'absolute',left:0,right:0,height:1,background:'linear-gradient(90deg,transparent,rgba(200,168,75,0.1),transparent)',animation:'gov-scan 12s linear infinite'}}/>
        </div>

        <div className="gov-page-wrap">
          <div style={{padding:'16px',border:'1px solid rgba(250,247,242,0.07)',background:'rgba(30,4,15,0.6)',marginBottom:20,position:'relative',overflow:'hidden',backdropFilter:'blur(2px)'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontFamily:MONO,fontSize:9,color:'rgba(250,247,242,0.3)',letterSpacing:2}}>// REGISTRATION PROGRESS</span>
              <span style={{fontFamily:MONO,fontSize:9,color:'#C8A84B'}}>{doneCount}/{ITEMS.length}</span>
            </div>
            <div style={{height:2,background:'rgba(250,247,242,0.07)',position:'relative',overflow:'hidden',marginBottom:8}}>
              <div style={{position:'absolute',inset:0,width:pct+'%',background:'linear-gradient(90deg,#C8A84B,#C8A84B)',boxShadow:'0 0 10px rgba(200,168,75,0.7)',transition:'width .8s cubic-bezier(.16,1,.3,1)'}}/>
            </div>
            <div style={{fontFamily:'monospace',fontSize:12,color:'rgba(250,247,242,0.42)',lineHeight:1.6}}>Tap any registration to expand. Tap the status badge to update progress.</div>
          </div>
