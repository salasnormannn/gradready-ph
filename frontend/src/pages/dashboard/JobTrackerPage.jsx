...
const CSS = `
  ...
  .t-row:hover{background:rgba(240,237,232,0.04)!important;border-left-color:#BE473D!important;}
  ...
  return(
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(28,5,18,0.88)',zIndex:600,display:'flex',alignItems:'flex-end',justifyContent:'center'}}
      ...
      <div className="modal-sheet" style={{
        background:'#2A0515',border:'1px solid rgba(240,237,232,0.1)',
        borderBottom:'none',
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:22}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'4px 12px 4px 8px',border:'1px solid rgba(240,237,232,0.1)'}}>
            ...
            <span style={{fontSize:9,letterSpacing:2,color:'rgba(240,237,232,0.4)'}}>APPLICATION</span>
          </div>
        ...
        <div style={{marginBottom:18}}>
          <div style={{fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.3)',letterSpacing:2,marginBottom:8}}>// STATUS</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
          <div>
            <div style={{fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.3)',letterSpacing:2,marginBottom:6}}>// MIN SALARY (PHP)</div>
            <Inp type="number" value={f.salaryMin} onChange={function(e){u('salaryMin',e.target.value)}} placeholder="35000"/>
          </div>
          <div>
            <div style={{fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.3)',letterSpacing:2,marginBottom:6}}>// MAX SALARY (PHP)</div>
            <Inp type="number" value={f.salaryMax} onChange={function(e){u('salaryMax',e.target.value)}} placeholder="50000"/>
          </div>
        </div>

        <div style={{marginBottom:20}}>
          <div style={{fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.3)',letterSpacing:2,marginBottom:6}}>// NOTES</div>
          <textarea value={f.notes} onChange={function(e){u('notes',e.target.value)}} placeholder="Notes, next steps..."
            rows={3} style={{width:'100%',padding:'12px 14px',background:'rgba(240,237,232,0.05)',border:'1px solid rgba(240,237,232,0.1)',color:'#F0EDE8',fontFamily:MONO,fontSize:11,resize:'none',letterSpacing:.3,transition:'border-color .18s'}}
            onFocus={function(e){e.target.style.borderColor='rgba(190,71,61,0.5)'}}
            onBlur={function(e){e.target.style.borderColor='rgba(240,237,232,0.1)'}} />
        </div>
    ...
    <div style={{borderBottom:'1px solid rgba(240,237,232,0.06)'}}>
      <div className="t-row" onClick={function(){setOpen(!open)}}
        ...
        <div style={{flex:1,minWidth:0}}>
          <div className="t-role" style={{fontFamily:MONO,fontSize:11,color:'rgba(240,237,232,0.7)',letterSpacing:.5,marginBottom:2,transition:'color .18s'}}>{app.role}</div>
        ...
        <div onClick={function(e){e.stopPropagation()}}>
          <select value={app.status} onChange={function(e){onStatusChange(app.id,e.target.value)}}
            style={{background:'rgba(240,237,232,0.05)',border:'1px solid rgba(240,237,232,0.1)',color:s.color,fontFamily:MONO,fontSize:9,letterSpacing:1,padding:'4px 6px',cursor:'pointer'}}>
      ...
      {open&&(
        <div style={{padding:'0 12px 14px 60px'}}>
          <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:app.notes?8:10}}>
            {app.workSetup&&<span style={{fontFamily:MONO,fontSize:8,color:'rgba(240,237,232,0.28)',padding:'2px 7px',border:'1px solid rgba(240,237,232,0.07)'}}>{app.workSetup}</span>}
            {app.salaryMin&&app.salaryMax&&<span style={{fontFamily:MONO,fontSize:8,color:'rgba(240,237,232,0.28)',padding:'2px 7px',border:'1px solid rgba(240,237,232,0.07)'}}>₱{Number(app.salaryMin).toLocaleString()}–{Number(app.salaryMax).toLocaleString()}</span>}
          </div>
          {app.notes&&<div style={{fontFamily:'monospace',fontSize:11,color:'rgba(240,237,232,0.3)',lineHeight:1.65,marginBottom:10,padding:'8px 10px',background:'rgba(240,237,232,0.03)',border:'1px solid rgba(240,237,232,0.06)'}}>{app.notes}</div>}
          <div style={{display:'flex',gap:8}}>
            {app.jobUrl&&(
              <button onClick={openJob} style={{flex:1,padding:'9px',background:'transparent',border:'1px solid rgba(240,237,232,0.1)',fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.45)',cursor:'pointer',letterSpacing:1,transition:'all .18s'}}
            ...
            <button onClick={function(){if(window.confirm('Remove this application?'))onDelete(app.id)}}
              style={{flex:1,padding:'9px',background:'transparent',border:'1px solid rgba(190,71,61,0.18)',fontFamily:MONO,fontSize:9,color:'rgba(190,71,61,0.55)',cursor:'pointer',letterSpacing:1,transition:'all .18s'}}
...
export default function JobTrackerPage(){
  ...
  return(
    <PageLayout title="JOB TRACKER" subtitle="// TRACK EVERY APPLICATION">
        ...
        {[{l:'TOTAL',v:stats.total||0},{l:'APPLIED',v:stats.applied||0},{l:'INTERVIEW',v:stats.interview||0},{l:'OFFERS',v:stats.offer||0},{l:'ACCEPTED',v:stats.accepted||0},{l:'REJECTED',v:stats.rejected||0}].map(function(c){
          return(
            <div key={c.l} style={{padding:'12px',background:'#3C091E'}}>
              <div style={{fontFamily:MONO,fontSize:8,color:'rgba(240,237,232,0.22)',letterSpacing:2,marginBottom:4}}>{c.l}</div>
        ...
        {['ALL'].concat(STATUSES.map(function(s){return s.key})).map(function(key){
          ...
          return(
            <button key={key} onClick={function(){setFilter(key)}}
              style={{flexShrink:0,padding:'5px 11px',border:'1px solid',borderColor:active?'#BE473D':'rgba(240,237,232,0.1)',background:'transparent',color:active?'#BE473D':'rgba(240,237,232,0.3)',fontFamily:MONO,fontSize:8,letterSpacing:1,cursor:'pointer',whiteSpace:'nowrap',transition:'all .15s'}}>
      ...
      {/* List */}
      {isLoading&&(
        <div style={{border:'1px solid rgba(240,237,232,0.06)',padding:28,textAlign:'center',fontFamily:MONO,fontSize:9,color:'rgba(240,237,232,0.2)',letterSpacing:2}}>LOADING...</div>
      )}
      {!isLoading&&filtered.length===0&&(
        <div style={{border:'1px solid rgba(240,237,232,0.06)',padding:32,textAlign:'center'}}>
          <div style={{fontFamily:MONO,fontSize:10,color:'rgba(240,237,232,0.3)',letterSpacing:1,marginBottom:6}}>
            {filter==='ALL'?'NO APPLICATIONS YET':'NO '+gs(filter).label.toUpperCase()+' APPLICATIONS'}
          </div>
          <div style={{fontFamily:'monospace',fontSize:10,color:'rgba(240,237,232,0.18)'}}>
