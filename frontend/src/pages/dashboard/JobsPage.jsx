...
const CSS = `
  ...
  .job-row:hover { background: rgba(240,237,232,0.04) !important; border-left-color: #BE473D !important; }
  .job-row:hover .job-title { color: #F0EDE8 !important; }
  .site-card { transition: all .22s; border-top: 2px solid transparent; }
  .site-card:hover { background: rgba(240,237,232,0.04) !important; }
  ...
  return (
    <div className="job-row" style={{ background:'transparent', padding:'16px 14px 16px 16px' }}>
      <div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:10 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8 }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div className="job-title" style={{ fontFamily:MONO, fontSize:12, color:'rgba(240,237,232,0.72)', letterSpacing:.4, marginBottom:2, transition:'color .18s' }}>
                {job.title}
              </div>
              <div style={{ fontFamily:'monospace', fontSize:9, color:'rgba(240,237,232,0.3)' }}>
            ...
            {job.fitScore != null && (
              <div style={{ textAlign:'right', flexShrink:0 }}>
                ...
                <div style={{ fontFamily:MONO, fontSize:7, color:'rgba(240,237,232,0.2)', letterSpacing:1 }}>MATCH%</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:10 }}>
        {job.employmentType && (
          <span style={{ fontFamily:MONO, fontSize:8, color:'rgba(240,237,232,0.28)', padding:'2px 7px', border:'1px solid rgba(240,237,232,0.08)', letterSpacing:.5 }}>{job.employmentType}</span>
        )}
        {job.salaryMin && job.salaryMax && (
          <span style={{ fontFamily:MONO, fontSize:8, color:'rgba(240,237,232,0.28)', padding:'2px 7px', border:'1px solid rgba(240,237,232,0.08)', letterSpacing:.5 }}>
      ...
      {/* Description snippet */}
      {job.description && (
        <div style={{ fontFamily:'monospace', fontSize:10, color:'rgba(240,237,232,0.28)', lineHeight:1.7, marginBottom:12 }}>
  ...
  return (
    <div style={{ border:'1px solid rgba(190,71,61,0.22)', background:'rgba(190,71,61,0.04)', padding:'20px' }}>
      ...
      <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'4px 12px 4px 8px', border:'1px solid rgba(190,71,61,0.3)', marginBottom:10 }}>
        <span style={{ fontSize:9, color:'#BE473D' }}>⚠</span>
        <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:2, color:'rgba(240,237,232,0.5)' }}>RED FLAG CHECKER</span>
      </div>

      <p style={{ fontFamily:'monospace', fontSize:10, color:'rgba(240,237,232,0.35)', lineHeight:1.75, marginBottom:16 }}>
        ...
        {[['text','PASTE JOB TEXT'],['url','PASTE JOB URL']].map(function(m) {
          ...
          return (
            <button key={m[0]} onClick={function() { setMode(m[0]); setValue('') }}
              style={{ padding:'10px', background:active?'#BE473D':'transparent', border:'none', color:active?'#F0EDE8':'rgba(240,237,232,0.35)', fontFamily:MONO, fontSize:9, letterSpacing:1.5, cursor:'pointer', transition:'all .18s' }}>
              {m[1]}
            </button>
          )
        })}
  ...
  return (
    <PageLayout title="JOB SEARCH" subtitle="// AI-MATCHED OPENINGS">
      ...
      <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'4px 12px 4px 8px', border:'1px solid rgba(240,237,232,0.1)', marginBottom:14 }}>
        ...
        <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:2.5, color:'rgba(240,237,232,0.38)' }}>TOP JOB SITES</span>
      </div>
        ...
        {JOB_SITES.map(function(site) {
          return (
            <a key={site.name} href={site.url} target="_blank" rel="noreferrer" className="site-card"
              ...
              <div style={{ fontFamily:MONO, fontSize:10, color:'#F0EDE8', letterSpacing:.5, marginBottom:4 }}>{site.name}</div>
              <div style={{ fontFamily:'monospace', fontSize:9, color:'rgba(240,237,232,0.28)' }}>{site.desc}</div>
            </a>
          )
        })}
      </div>
      ...
      <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'4px 12px 4px 8px', border:'1px solid rgba(240,237,232,0.1)', marginBottom:12 }}>
        ...
        <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:2.5, color:'rgba(240,237,232,0.38)' }}>SEARCH JOBS</span>
      </div>
      ...
      <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'4px 12px 4px 8px', border:'1px solid rgba(240,237,232,0.1)', marginBottom:14 }}>
        ...
        <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:2.5, color:'rgba(240,237,232,0.38)' }}>
      ...
      {isError && (
        <div style={{ border:'1px solid rgba(190,71,61,0.22)', background:'rgba(190,71,61,0.05)', padding:'16px', marginBottom:24 }}>
          <div style={{ fontFamily:MONO, fontSize:10, color:'#BE473D', letterSpacing:1, marginBottom:4 }}>COULD NOT LOAD JOBS</div>
          <div style={{ fontFamily:'monospace', fontSize:10, color:'rgba(240,237,232,0.35)' }}>Make sure the backend is running and try again.</div>
        </div>
      )}
      ...
      {!isLoading && !isError && jobs.length === 0 && (
        <div style={{ border:'1px solid rgba(240,237,232,0.06)', padding:32, textAlign:'center', marginBottom:24 }}>
          <div style={{ fontFamily:MONO, fontSize:10, color:'rgba(240,237,232,0.25)', letterSpacing:1, marginBottom:6 }}>NO JOBS FOUND</div>
          <div style={{ fontFamily:'monospace', fontSize:10, color:'rgba(240,237,232,0.18)' }}>Try a different search or check back later.</div>
        </div>
      )}
      ...
      {/* ── RED FLAG CHECKER ── */}
      <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'4px 12px 4px 8px', border:'1px solid rgba(240,237,232,0.1)', marginBottom:14 }}>
        <span style={{ fontSize:9, color:'#BE473D' }}>04</span>
        <span style={{ fontFamily:MONO, fontSize:9, letterSpacing:2.5, color:'rgba(240,237,232,0.38)' }}>RED FLAG CHECKER</span>
      </div>
