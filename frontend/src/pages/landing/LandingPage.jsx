...
// ─── Fonts + global styles ─────────────────────────────────────────────────
const GLOBAL_CSS = `
  @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;900&family=Share+Tech+Mono&display=swap');
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  html { scroll-behavior:smooth; overflow-x:hidden; width:100%; }
  body { background:#0F2044; overflow-x:hidden; width:100%; }
  ...
  .feat-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:1px; background:rgba(15,32,68,0.08); }
...
function Tag({ num, label, dark = true }) {
  const fg = dark ? '#FAF7F2' : '#0F2044'
  const border = dark ? 'rgba(250,247,242,0.2)' : 'rgba(15,32,68,0.2)'
  return (
      <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:20 }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'5px 14px 5px 10px', border:'1px solid '+border }}>
          {num && <span style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'#C8A84B', marginRight:2 }}>{num}</span>}
          <span style={{ fontFamily:'Inter, sans-serif', fontSize:12, letterSpacing:2, color:fg }}>{label}</span>
        </div>
        <div style={{ display:'flex', gap:3 }}>
          <div style={{ width:2, height:18, background:dark?'rgba(250,247,242,0.35)':'rgba(15,32,68,0.35)' }} />
          <div style={{ width:2, height:18, background:dark?'rgba(250,247,242,0.35)':'rgba(15,32,68,0.35)' }} />
        </div>
      </div>
  )
...
function Heading({ lines, dark=true, size='clamp(44px,7vw,88px)', center=false }) {
  const color = dark ? '#FAF7F2' : '#0F2044'
  return (
      <div style={{ textAlign:center?'center':'left' }}>
        {lines.map((line, i) => (
            <div key={i} style={{ position:'relative', display:'block' }}>
              {i===1 && <div style={{ position:'absolute', bottom:2, left:center?'50%':0, transform:center?'translateX(-50%)':'none', width:72, height:2, background:'#C8A84B' }} />}
              <span style={{ fontFamily:'Inter, sans-serif', fontWeight:900, fontSize:size, color:color, letterSpacing:'-1px', lineHeight:.92, display:'block', whiteSpace:'nowrap' }}>
            {line}
          </span>
            </div>
        ))}
...
function Cube({ x, y, size=44, rot=0, anim='floatA', delay=0, label=null, className='' }) {
  return (
      <div className={className} style={{ position:'absolute', left:x+'%', top:y+'%', pointerEvents:'none', animation:anim+' '+(6+delay)+'s ease-in-out infinite', animationDelay:delay+'s' }}>
        <div style={{ width:size, height:size, background:'rgba(15,32,68,0.07)', borderRadius:8, transform:'rotate('+rot+'deg)', position:'relative' }}>
          {label && (
              <div style={{ position:'absolute', top:'115%', left:0, fontFamily:'Inter, sans-serif', fontSize:9, color:'rgba(15,32,68,0.22)', letterSpacing:1, whiteSpace:'nowrap', lineHeight:1.6 }}>
                <div>{label}</div>
                <div style={{ fontSize:8, color:'rgba(15,32,68,0.14)' }}>IS · 01 · 02 · 03</div>
              </div>
          )}
        </div>
      </div>
...
function DiagLine({ x, y, angle, length=200, dark=false }) {
  const color = dark ? 'rgba(250,247,242,0.06)' : 'rgba(15,32,68,0.06)'
  return (
...
function Loader({ onDone }) {
  ...
  return (
      <div style={{ position:'fixed', inset:0, background:'#0F2044', zIndex:9999, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:'Share Tech Mono, monospace' }}>
        {/* Corner brackets */}
        {[{t:20,l:20,tr:'none',bt:'top',bl:'left'},{t:20,r:20,tr:'none',bt:'top',bl:'right'},{b:20,l:20,tr:'none',bt:'bottom',bl:'left'},{b:20,r:20,tr:'none',bt:'bottom',bl:'right'}].map((pos,i) => (
            <div key={i} style={{ position:'absolute', top:pos.t, bottom:pos.b, left:pos.l, right:pos.r, width:24, height:24, borderTop:pos.bt==='top'?'1px solid rgba(250,247,242,0.25)':'none', borderBottom:pos.bt==='bottom'?'1px solid rgba(250,247,242,0.25)':'none', borderLeft:pos.bl==='left'?'1px solid rgba(250,247,242,0.25)':'none', borderRight:pos.bl==='right'?'1px solid rgba(250,247,242,0.25)':'none' }} />
        ))}

        <div style={{ textAlign:'center', marginBottom:64 }}>
          <div style={{ fontSize:'clamp(22px,4vw,36px)', color:'#FAF7F2', letterSpacing:4, marginBottom:8 }}>{line1}</div>
          <div style={{ fontSize:'clamp(14px,2.5vw,22px)', color:'rgba(250,247,242,0.4)', letterSpacing:3 }}>{line2}</div>
        </div>

        <div style={{ position:'absolute', bottom:48, left:'50%', transform:'translateX(-50%)', textAlign:'center', width:240 }}>
          <div style={{ fontSize:12, color:'rgba(250,247,242,0.3)', letterSpacing:3, marginBottom:12 }}>
...
export default function LandingPage() {
  ...
  return (
      <div style={{ fontFamily:'Inter, sans-serif', background:'#0F2044', color:'#0F2044', overflowX:'hidden', width:'100%', maxWidth:'100vw' }}>
        <style>{GLOBAL_CSS}</style>

        {/* ── NAV ── */}
        <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, height:62, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 40px', transition:'all .4s', background:'rgba(250,247,242,0.97)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(15,32,68,0.08)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }} onClick={() => window.scrollTo({top:0,behavior:'smooth'})}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:2 }}>
              {[0,1,2,3].map(i=><div key={i} style={{ width:7, height:7, background:i<2?'#0F2044':'#C8A84B', borderRadius:1 }} />)}
            </div>
            <span style={{ fontSize:15, color:'#0F2044', letterSpacing:3 }}>GRADREADY</span>
          </div>
          <div className="nav-links">
            {[['FEATURES','feat'],['HOW IT WORKS','how'],['KUYA AI','kuya']].map(([l,id])=>(
                <button key={id} onClick={()=>document.getElementById(id)?.scrollIntoView({behavior:'smooth'})} style={{ background:'none', border:'none', color:'#7A7060', fontSize:11, letterSpacing:2.5, cursor:'pointer', transition:'color .2s', fontFamily:'Inter, sans-serif' }} onMouseEnter={e=>e.currentTarget.style.color='#0F2044'} onMouseLeave={e=>e.currentTarget.style.color='#7A7060'}>{l}</button>
            ))}
            <div style={{ width:1, height:14, background:'rgba(15,32,68,0.18)' }} />
            <button onClick={signIn} style={{ background:'#0F2044', border:'none', color:'#FAF7F2', fontSize:11, letterSpacing:2.5, cursor:'pointer', fontFamily:'Inter, sans-serif', padding:'9px 18px' }}>SIGN IN</button>
          </div>
          <button onClick={go} style={{ background:'#0F2044', color:'#FAF7F2', border:'none', padding:'9px 22px', fontSize:11, letterSpacing:2, cursor:'pointer', fontFamily:'Inter, sans-serif', transition:'background .25s' }} onMouseEnter={e=>e.currentTarget.style.background='#C8A84B'} onMouseLeave={e=>e.currentTarget.style.background='#0F2044'}>GET STARTED</button>
        </nav>
      ...
      ══════════════════════════════════════════════════════════════════ */}
        <section className="hero-split" style={{ position:'relative', overflow:'hidden', width:'100%', maxWidth:'100vw' }}>
          ...
          {/* Left — deep maroon */}
          <div className="hero-left" style={{ background:'#0F2044', display:'flex', flexDirection:'column', justifyContent:'flex-end', position:'relative', overflow:'hidden' }}>
            {/* Grid pattern */}
            <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(250,247,242,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(250,247,242,0.04) 1px,transparent 1px)', backgroundSize:'55px 55px', pointerEvents:'none' }} />
            {/* Radial glow from center */}
            <div style={{ position:'absolute', top:'40%', right:'-15%', width:500, height:500, borderRadius:'50%', background:'radial-gradient(circle,rgba(200,168,75,0.18) 0%,transparent 70%)', pointerEvents:'none', transform:`translate(${(mouseX-0.5)*20}px,${(mouseY-0.5)*20}px)`, transition:'transform .1s ease' }} />

            {/* Binary decorations */}
            {[['8%','18%','10100110_'],['62%','28%','01010011_'],['15%','55%','01010101'],['48%','72%','00010000_']].map(([x,y,v],i)=>(
                <div key={i} style={{ position:'absolute', left:x, top:y, fontFamily:'Inter, sans-serif', fontSize:10, color:'rgba(250,247,242,0.1)', letterSpacing:2, pointerEvents:'none', userSelect:'none' }}>{v}</div>
            ))}
            ...
            {[[30,45],[70,20],[55,65]].map(([x,y],i)=>(
                <div key={i} style={{ position:'absolute', left:x+'%', top:y+'%', color:'rgba(250,247,242,0.12)', fontSize:14, pointerEvents:'none' }}>+</div>
            ))}
            ...
            <div className="h1" style={{ position:'absolute', top:'8%', right:'-12%', perspective:600, zIndex:2 }}>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, transform:'rotateX(20deg) rotateY(-25deg)', transformStyle:'preserve-3d', animation:'floatA 8s ease-in-out infinite' }}>
                {Array(9).fill(0).map((_,i)=>(
                    <div key={i} style={{ width:56, height:34, background:`linear-gradient(135deg, #1E4A9E ${i%3*10}%, #0F2044)`, borderRadius:7, boxShadow:'0 4px 12px rgba(0,0,0,0.4)', opacity:0.65+((i%3)*0.12), transform:`translateZ(${i*5}px)` }} />
                ))}
              </div>
            </div>
            ...
            <div className="h4" style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
              <button onClick={go} style={{ background:'transparent', color:'#FAF7F2', border:'1px solid rgba(250,247,242,0.35)', padding:'14px 32px', fontFamily:'Inter, sans-serif', fontSize:12, letterSpacing:2, cursor:'pointer', transition:'all .25s' }} onMouseEnter={e=>{e.currentTarget.style.background='#C8A84B';e.currentTarget.style.borderColor='#C8A84B'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.borderColor='rgba(250,247,242,0.35)'}}>
                START FREE
              </button>
              <button onClick={()=>document.getElementById('feat')?.scrollIntoView({behavior:'smooth'})} style={{ background:'none', border:'none', color:'rgba(250,247,242,0.4)', fontSize:11, letterSpacing:2, cursor:'pointer', fontFamily:'Inter, sans-serif', transition:'color .2s' }} onMouseEnter={e=>e.currentTarget.style.color='rgba(250,247,242,0.8)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(250,247,242,0.4)'}>
                EXPLORE ↓
              </button>
            </div>
          </div>
          ...
          <div className="stats-right" style={{ background:'#FAF7F2', display:'flex', flexDirection:'column', justifyContent:'flex-end', position:'relative' }}>
            ...
            <div className="h5">
              {[['7','GOV REGISTRATIONS COVERED'],['GPT-4o','POWERED KUYA AI IN TAGLISH'],['50+','REAL PH SALARY DATA POINTS']].map(([v,l],i)=>(
                  <div key={i} style={{ display:'flex', gap:20, alignItems:'flex-start', padding:'18px 0', borderBottom:i<2?'1px solid rgba(15,32,68,0.07)':'none' }}>
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:26, fontWeight:700, color:'#C8A84B', lineHeight:1, minWidth:72, flexShrink:0 }}>{v}</span>
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:10, color:'rgba(15,32,68,0.4)', lineHeight:1.7, paddingTop:5, letterSpacing:1 }}>{l}</span>
                  </div>
              ))}
            </div>
          </div>
        ...
        {/* ── MARQUEE ── */}
        <div style={{ background:'#1E4A9E', borderTop:'1px solid rgba(15,32,68,0.2)', borderBottom:'1px solid rgba(15,32,68,0.2)', padding:'13px 0', overflow:'hidden' }}>
          <div style={{ display:'flex', animation:'mq 28s linear infinite', whiteSpace:'nowrap' }}>
            {Array(4).fill(['TIN REGISTRATION','·','SSS','·','PHILHEALTH','·','PAG-IBIG','·','NBI CLEARANCE','·','PHILSYS ID','·','PRC BOARD EXAM','·','KUYA AI','·','JOB MATCHING','·','COVER LETTER','·','SALARY GUIDE','·']).flat().map((item,i)=>(
                <span key={i} style={{ fontFamily:'Inter, sans-serif', fontSize:11, letterSpacing:2.5, color:item==='·'?'rgba(255,255,255,0.45)':'#FFFFFF', marginRight:item==='·'?18:26 }}>{item}</span>
            ))}
          </div>
        </div>
      ...
      ══════════════════════════════════════════════════════════════════ */}
        <section className="split-2col" style={{ minHeight:'90vh', background:'#0F2044', position:'relative', overflow:'hidden' }}>
          {/* Grid + diagonals on left */}
          <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(250,247,242,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(250,247,242,0.03) 1px,transparent 1px)', backgroundSize:'55px 55px', pointerEvents:'none' }} />
          ...
          {[[15,30],[45,55],[70,25],[80,70]].map(([x,y],i)=>(
              <div key={i} style={{ position:'absolute', left:x+'%', top:y+'%', color:'rgba(250,247,242,0.08)', fontSize:16, pointerEvents:'none' }}>+</div>
          ))}
          ...
          <div style={{ position:'absolute', top:0, left:'25%', right:'25%', bottom:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none', zIndex:2 }}>
            <div style={{ width:90, height:'70vh', background:'linear-gradient(180deg,#1E4A9E 0%,#0F2044 40%,#1E4A9E 70%,#0F2044 100%)', borderRadius:12, boxShadow:'inset -8px 0 20px rgba(0,0,0,0.5), inset 4px 0 12px rgba(200,168,75,0.4), 0 0 60px rgba(200,168,75,0.12)', animation:'floatB 6s ease-in-out infinite', transform:'rotate(-5deg)' }}>
              {/* Diamond pip at top */}
              <div style={{ position:'absolute', top:-12, left:'50%', transform:'translateX(-50%)', width:14, height:14, background:'#FAF7F2', rotate:'45deg' }} />
            </div>
          </div>
          ...
          <div style={{ padding:'80px 40px 60px', display:'flex', flexDirection:'column', justifyContent:'center', gap:24, zIndex:3 }}>
            {[
              ...
              ['GOV REGISTRATION GUIDE','Step-by-step for TIN, SSS, PhilHealth, Pag-IBIG, NBI, PRC, and PhilSys — with exact fees and forms.'],
                ...
                <div key={i} className={'rev d'+(i+1)} style={{ padding:'20px 24px', border:'1px solid rgba(250,247,242,0.08)', background:'rgba(250,247,242,0.04)', transition:'border-color .3s' }} onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(200,168,75,0.4)'} onMouseLeave={e=>e.currentTarget.style.borderColor='rgba(250,247,242,0.08)'}>
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                    <div style={{ width:8, height:8, background:'#C8A84B' }} />
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'#FAF7F2', letterSpacing:2 }}>{title}</span>
                  </div>
                  <p style={{ fontFamily:'Inter, sans-serif', fontSize:12, color:'rgba(250,247,242,0.4)', lineHeight:1.8, letterSpacing:.3 }}>{desc}</p>
                </div>
            ))}
          </div>
        </section>
      ...
      ══════════════════════════════════════════════════════════════════ */}
        <section id="feat" style={{ background:'#FAF7F2', minHeight:'90vh', position:'relative', padding:'100px 40px', overflow:'hidden' }}>
          ...
          {([ ['3%','15%','// CAREER DATA'], ['72%','22%','// VECTOR SEARCH'], ['8%','82%','// LLM PIPELINE'], ['85%','75%','// GRAD ANALYTICS'] ]).map(([x,y,l],i)=>(
              <div key={i} style={{ position:'absolute', left:x, top:y, fontFamily:'Inter, sans-serif', fontSize:9, color:'rgba(15,32,68,0.2)', letterSpacing:1.5, lineHeight:1.6, pointerEvents:'none' }}>
          ...
          <div className="feat-grid" style={{ maxWidth:1100, margin:'0 auto', position:'relative', zIndex:2 }}>
            {[
              ...
              {n:'06',tag:'BOARD EXAM',title:'PRC\nTRACKER',desc:'Schedules for all licensed professions. AI generates a study plan from your exam date.'},
                ...
                <div key={i} className={'rev d'+(i%3+1)} style={{ background:'#FAF7F2', padding:'32px 28px', borderTop:'2px solid transparent', transition:'all .3s', cursor:'default' }} onMouseEnter={e=>{e.currentTarget.style.borderTopColor='#C8A84B';e.currentTarget.style.background='rgba(255,255,255,0.6)'}} onMouseLeave={e=>{e.currentTarget.style.borderTopColor='transparent';e.currentTarget.style.background='#FAF7F2'}}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:28 }}>
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'#C8A84B' }}>{f.n}</span>
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:10, color:'rgba(15,32,68,0.3)', padding:'3px 10px', border:'1px solid rgba(15,32,68,0.12)', letterSpacing:1 }}>{f.tag}</span>
                  </div>
                  <div style={{ fontFamily:'Inter, sans-serif', fontSize:18, color:'#0F2044', lineHeight:1.15, marginBottom:14, whiteSpace:'pre-line', letterSpacing:.5 }}>{f.title}</div>
                  <p style={{ fontFamily:'Inter, sans-serif', fontSize:12, color:'rgba(15,32,68,0.45)', lineHeight:1.8, letterSpacing:.3 }}>{f.desc}</p>
                </div>
            ))}
          </div>
        </section>
      ...
      ══════════════════════════════════════════════════════════════════ */}
        <section id="how" style={{ background:'#FAF7F2', position:'relative', padding:'100px 40px', overflow:'hidden' }}>
          {/* More floating cubes */}
          ...
          {[['3%','18%','// AI-FIRST TOOL'],['75%','30%','// VECTOR SEARCH'],['12%','75%','// GRAD ANALYTICS']].map(([x,y,l],i)=>(
              <div key={i} style={{ position:'absolute', left:x, top:y, fontFamily:'Inter, sans-serif', fontSize:9, color:'rgba(15,32,68,0.18)', letterSpacing:1.5, lineHeight:1.6, pointerEvents:'none' }}>{l}<br/><span style={{ fontSize:8 }}>IS · 01 · 02 · 03</span></div>
          ))}
          ...
          <div style={{ maxWidth:900, margin:'0 auto', display:'flex', flexDirection:'column', gap:1, background:'rgba(15,32,68,0.07)', position:'relative', zIndex:2 }}>
            {[
              ...
              {n:'04',tag:'ACTING',title:'TAKE ACTION',desc:'Register, apply, track your applications, and ask Kuya AI — all in one place.'},
                ...
                <div key={i} className={'rev d'+(i%2+1)} style={{ background:'#FAF7F2', padding:'48px 40px', textAlign:'center', borderLeft:'2px solid transparent', transition:'all .3s' }} onMouseEnter={e=>{e.currentTarget.style.borderLeftColor='#C8A84B'}} onMouseLeave={e=>{e.currentTarget.style.borderLeftColor='transparent'}}>
                  <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:12, marginBottom:20 }}>
                    <span style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'#C8A84B', padding:'4px 10px', border:'1px solid rgba(200,168,75,0.3)' }}>{s.n}</span>
                    <Tag num={null} label={s.tag} dark={false} />
                  </div>
                  <div style={{ fontFamily:'Inter, sans-serif', fontSize:'clamp(24px,4vw,44px)', color:'#0F2044', lineHeight:.95, letterSpacing:'-0.5px', marginBottom:20, fontWeight:900 }}>{s.title}</div>
                  <p style={{ fontFamily:'Inter, sans-serif', fontSize:13, color:'rgba(15,32,68,0.45)', lineHeight:1.8, maxWidth:480, margin:'0 auto', letterSpacing:.3 }}>{s.desc}</p>
                </div>
            ))}
          </div>
        </section>
      ...
      ══════════════════════════════════════════════════════════════════ */}
        <section id="kuya" className="kuya-split" style={{ minHeight:'90vh', background:'#0F2044', position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(250,247,242,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(250,247,242,0.03) 1px,transparent 1px)', backgroundSize:'55px 55px', pointerEvents:'none' }} />
          <DiagLine x={0} y={25} angle={-15} length={350} dark />
          <DiagLine x={30} y={70} angle={10} length={280} dark />
          {[[20,35],[65,20],[75,60],[10,75]].map(([x,y],i)=>(
              <div key={i} style={{ position:'absolute', left:x+'%', top:y+'%', color:'rgba(250,247,242,0.07)', fontSize:16, pointerEvents:'none' }}>+</div>
          ))}
          {[['8%','12%','10100110_'],['55%','35%','01010011'],['20%','65%','00010000_']].map(([x,y,v],i)=>(
              <div key={i} style={{ position:'absolute', left:x, top:y, fontFamily:'Inter, sans-serif', fontSize:9, color:'rgba(250,247,242,0.08)', letterSpacing:2, pointerEvents:'none', userSelect:'none' }}>{v}</div>
          ))}
          ...
          <div style={{ padding:'80px 40px 64px', display:'flex', flexDirection:'column', justifyContent:'flex-end', zIndex:2 }}>
            ...
            <p className="rev d2" style={{ fontFamily:'Inter, sans-serif', fontSize:12, color:'rgba(250,247,242,0.4)', lineHeight:1.9, letterSpacing:.3, maxWidth:380, marginBottom:36 }}>
            ...
            {['PAANO MAG-REGISTER SA PHILHEALTH?','CHECK JOB POSTING FOR RED FLAGS','WRITE ME A COVER LETTER','EXPLAIN MY SSS DEDUCTIONS'].map((q,i)=>(
                <div key={i} className="rev" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', border:'1px solid rgba(250,247,242,0.07)', fontFamily:'Inter, sans-serif', fontSize:11, color:'rgba(250,247,242,0.35)', letterSpacing:.5, marginBottom:6, cursor:'default', transition:'all .25s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(200,168,75,0.4)';e.currentTarget.style.color='#FAF7F2';e.currentTarget.style.paddingLeft='18px'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(250,247,242,0.07)';e.currentTarget.style.color='rgba(250,247,242,0.35)';e.currentTarget.style.paddingLeft='14px'}}>
                  <span>"{q}"</span><span style={{ color:'#C8A84B' }}>→</span>
                </div>
            ))}
          </div>
          ...
          <div style={{ padding:'80px 40px 64px 56px', display:'flex', alignItems:'center', zIndex:2 }}>
            <div className="rev d2" style={{ width:'100%', border:'1px solid rgba(250,247,242,0.1)', background:'rgba(250,247,242,0.03)' }}>
              <div style={{ padding:'14px 18px', borderBottom:'1px solid rgba(250,247,242,0.08)', display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:32, height:32, background:'#C8A84B', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>🤙</div>
                <div>
                  <div style={{ fontFamily:'Inter, sans-serif', fontSize:13, color:'#FAF7F2', letterSpacing:1 }}>KUYA AI</div>
                  <div style={{ fontFamily:'Inter, sans-serif', fontSize:9, color:'#1E8449', letterSpacing:1.5 }}>● ONLINE</div>
                </div>
              </div>
              <div style={{ padding:'18px', display:'flex', flexDirection:'column', gap:12 }}>
                {[{f:'k',t:"Uy! Kamusta? I'm Kuya AI — your career big brother. Anong kailangan mo ngayon?"},{f:'u',t:'How do I register for SSS?'},{f:'k',t:"Madali lang! Go to my.sss.gov.ph, click 'Not yet registered'. Fill out your details with valid ID. SS Number arrives via email in 1-3 days. Libre yan!"}].map((m,i)=>(
                    <div key={i} style={{ display:'flex', justifyContent:m.f==='u'?'flex-end':'flex-start', gap:8 }}>
                      ...
                      <div style={{ maxWidth:'82%', padding:'10px 14px', background:m.f==='u'?'#C8A84B':'rgba(250,247,242,0.06)', border:m.f==='k'?'1px solid rgba(250,247,242,0.08)':'none', fontFamily:'Inter, sans-serif', fontSize:11, color:'#FAF7F2', lineHeight:1.7 }}>{m.t}</div>
                    </div>
                ))}
              </div>
              <div style={{ padding:'12px 18px', borderTop:'1px solid rgba(250,247,242,0.08)', display:'flex', gap:8 }}>
                <div style={{ flex:1, padding:'10px 14px', border:'1px solid rgba(250,247,242,0.1)', fontFamily:'Inter, sans-serif', fontSize:11, color:'rgba(250,247,242,0.2)', background:'transparent' }}>Magtanong kay Kuya AI...</div>
                <div style={{ width:34,height:34,background:'#C8A84B',display:'flex',alignItems:'center',justifyContent:'center',color:'#FAF7F2',fontSize:14 }}>↑</div>
              </div>
            </div>
          </div>
        </section>
        ...
        {/* ══════════════════════════════════════════════════════════════════
          CTA — dark section
      ══════════════════════════════════════════════════════════════════ */}
        <section style={{ background:'#0F2044', padding:'100px 40px', position:'relative', overflow:'hidden' }}>
          <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(250,247,242,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(250,247,242,0.03) 1px,transparent 1px)', backgroundSize:'55px 55px', pointerEvents:'none' }} />
          ...
          {[['5%','15%','10110011'],['80%','25%','01001010_'],['12%','80%','00010001']].map(([x,y,v],i)=>(
              <div key={i} style={{ position:'absolute', left:x, top:y, fontFamily:'Inter, sans-serif', fontSize:9, color:'rgba(250,247,242,0.08)', letterSpacing:2, pointerEvents:'none' }}>{v}</div>
          ))}
          ...
          <div className="cta-split" style={{ maxWidth:1200, margin:'0 auto', position:'relative', zIndex:2 }}>
            <div style={{ width:'100%' }}>
              ...
              <p className="rev d2" style={{ fontFamily:'Inter, sans-serif', fontSize:12, color:'rgba(250,247,242,0.4)', lineHeight:1.9, maxWidth:440, marginTop:28, letterSpacing:.3 }}>
            ...
            <div className="rev d2 cta-buttons">
              <button onClick={go} style={{ background:'#FAF7F2', color:'#0F2044', border:'none', padding:'18px 40px', fontFamily:'Inter, sans-serif', fontSize:12, letterSpacing:2, cursor:'pointer', transition:'all .25s', display:'block', whiteSpace:'nowrap' }} onMouseEnter={e=>e.currentTarget.style.background='#C8A84B'} onMouseLeave={e=>e.currentTarget.style.background='#FAF7F2'}>
                CREATE FREE ACCOUNT
              </button>
              <button onClick={signIn} style={{ background:'transparent', color:'rgba(250,247,242,0.5)', border:'1px solid rgba(250,247,242,0.18)', padding:'17px 40px', fontFamily:'Inter, sans-serif', fontSize:12, letterSpacing:2, cursor:'pointer', transition:'all .25s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(250,247,242,0.5)';e.currentTarget.style.color='#FAF7F2'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(250,247,242,0.18)';e.currentTarget.style.color='rgba(250,247,242,0.5)'}}>
                SIGN IN
              </button>
            </div>
          </div>
        ...
        {/* ── FOOTER ── */}
        <footer style={{ background:'#0A1832', padding:'56px 40px 32px', borderTop:'1px solid rgba(250,247,242,0.05)' }}>
          <div style={{ maxWidth:1200, margin:'0 auto' }}>
            <div className="footer-grid" style={{ marginBottom:56 }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:18 }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:2 }}>
                    {[0,1,2,3].map(i=><div key={i} style={{ width:6,height:6,background:i<2?'#C8A84B':'#C8A84B',borderRadius:1 }} />)}
                  </div>
                  <span style={{ fontFamily:'Inter, sans-serif', fontSize:14, color:'#FAF7F2', letterSpacing:3 }}>GRADREADY</span>
                </div>
                <p style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'rgba(250,247,242,0.28)', lineHeight:1.9, maxWidth:220, letterSpacing:.3 }}>The career companion for Filipinos navigating professional beginnings. Made in the Philippines.</p>
              </div>
              {[{t:'FEATURES',l:['ROADMAP','GOV REGS','KUYA AI','JOB SEARCH','FINANCE','BOARD EXAMS']},{t:'RESOURCES',l:['BIR PORTAL','SSS ONLINE','PHILHEALTH','PAG-IBIG','NBI CLEARANCE','PRC WEBSITE']},{t:'COMPANY',l:['ABOUT','PRIVACY','TERMS','CONTACT']}].map((col,i)=>(
                  <div key={i}>
                    <div style={{ fontFamily:'Inter, sans-serif', fontSize:9, fontWeight:700, letterSpacing:2.5, color:'rgba(250,247,242,0.2)', marginBottom:18 }}>{col.t}</div>
                    <div style={{ display:'flex', flexDirection:'column', gap:11 }}>
                      {col.l.map((l,j)=><span key={j} style={{ fontFamily:'Inter, sans-serif', fontSize:11, color:'rgba(250,247,242,0.32)', cursor:'pointer', letterSpacing:.5, transition:'color .2s' }} onMouseEnter={e=>e.currentTarget.style.color='#FAF7F2'} onMouseLeave={e=>e.currentTarget.style.color='rgba(250,247,242,0.32)'}>{l}</span>)}
                    </div>
                  </div>
              ))}
            </div>
            <div style={{ height:1, background:'rgba(250,247,242,0.06)', marginBottom:24 }} />
            <div className="footer-bottom">
              <span style={{ fontFamily:'Inter, sans-serif', fontSize:10, color:'rgba(250,247,242,0.18)', letterSpacing:.5 }}>© 2025 GRADREADY PH · MADE IN THE PHILIPPINES 🇵🇭</span>
              <span style={{ fontFamily:'Inter, sans-serif', fontSize:10, color:'rgba(250,247,242,0.18)', display:'flex', gap:8, alignItems:'center' }}>
