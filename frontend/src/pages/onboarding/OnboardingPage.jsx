...
export default function OnboardingPage() {
    ...
    function canNext() {
        if (step === 1) return showOther ? data.course.trim().length > 0 : !!data.course
        if (step === 2) {
                    ...
                    {step === 1 && (
                        <div>
                            ...
                            <div className="list-scroll">
                                {COURSES.map((c, i) => {
                                    var isOther = c === 'Other'
                                    var selected = isOther ? showOther : (!showOther && data.course === c)
                                    return (
                                        <button key={c} className={'opt-row' + (selected ? ' sel' : '')}
                                                onClick={() => {
                                                    if (isOther) {
                                                        setShowOther(true)
                                                        update('course', '')
